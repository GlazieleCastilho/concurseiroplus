...
export type LessonCommentTreeUpdater = (updater: (tree: LessonCommentItem[]) => LessonCommentItem[]) => void;

function insertReply(tree: LessonCommentItem[], parentId: string, reply: LessonCommentItem): LessonCommentItem[] {
...
function removeComment(tree: LessonCommentItem[], commentId: string): LessonCommentItem[] {
...
export function LessonCommentNode({
  comment,
  lessonId,
  currentUserId,
  depth,
  ...
  depth: number;
  onChanged: LessonCommentTreeUpdater;
}) {
  ...
  async function handleReply(event: React.FormEvent) {
    ...
    try {
      ...
      const { comment: reply } = (await res.json()) as { comment: LessonCommentItem };
  ...
  return (
    <div className={`space-y-2 ${depth > 0 ? indentClass : ""}`}>
      ...
      {comment.replies.map((reply) => (
        <LessonCommentNode key={reply.id} comment={reply} lessonId={lessonId} currentUserId={currentUserId} depth={depth + 1} onChanged={onChanged} />
      ))}
    </div>
  );
}
